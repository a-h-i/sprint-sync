...
describe('AuthController (e2e)', () => {
  ...
  describe('/auth/login (POST)', () => {
    ...
    it('should return 200 if user and password are correct', async () => {
      ...
      expect(parsedBody.user.profile.last_name).toStrictEqual(
        profile.last_name,
      );
      expect(parsedBody.user.profile.first_name).toStrictEqual(
        profile.first_name,
      );
      const tokenIsValid: Record<string, unknown> =
        await jwtService.verifyAsync(parsedBody.access_token);

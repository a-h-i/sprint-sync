...
describe('ProfileController (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const loginData = await loginService.signIn(
            user.username,
            'password',
        );
    ...
    describe('/profile/:id (PUT)', () => {
        ...
        it('updates user', async () => {
            ...
            const response = await request(app.getHttpServer())
                ...
                .send(profileData)
                .expect(200);
            const changedProfile = await source.manager.findOneByOrFail<Profile>(Profile, {
                user_id: userId
            });
    ...
    describe('/profile (GET)', () => {
        ...
        it('gets user list', async () => {
            ...
            const parsedFirstPage = plainToInstance(ListUsersResponseDto, firstPageResponse.body);
            ...
            const parsedSecondPage = plainToInstance(ListUsersResponseDto, secondPageResponse.body);
    ...
    describe('/profile/:id/password (PATCH)', () => {
        ...
        it('changes password', async () => {
            ...
            const loginInfo = await app.get(LoginService).signIn(user.username, 'password');
            await request(app.getHttpServer())
                .patch(`/profile/${user.id}/password`)
                .send(data)
                .set('Authorization', `Bearer ${loginInfo.access_token}`)
                .expect(200);
            expect(() => app.get(LoginService).signIn(user.username, 'password2')).not.toThrow();
        });

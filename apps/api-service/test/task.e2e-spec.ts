...
describe('TaskController (e2e)', () => {
    ...
    it('requires authentication for all task routes', async () => {
        ...
        await request(app.getHttpServer()).put(`/task/${taskId}`).send({}).expect(401);
    });
    ...
    describe('/task/:id (GET)', () => {
        it('gets task by ID', async () => {
            const task = await FactoryGirl.create<Task>('Task', { assigned_to_user_id: user.id });
    ...
    describe('/task/:id (PUT)', () => {
        it('updates a task', async () => {
            ...
            expect(response.status).toBe(200);
            const updatedTask = await source.manager.findOneOrFail(Task, { where: { id: task.id } });
            ...
            expect(updatedTask.assigned_to_user_id).toStrictEqual(updates.assigned_to_user_id);
        });
    ...
    describe('/task (GET)', () => {
        ...
        it('gets task list', async () => {
            ...
            const parsedResponse = plainToInstance(ListTasksResponseDto, response.body);

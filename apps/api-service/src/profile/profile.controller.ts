...
export class ProfileController {
    ...
    @UseGuards(AuthGuard)
    @Put(':id')
    async updateUser(@Param('id') id: string, @Body() body: ProfileDto, @CurrentUser() currentUser: User): Promise<UserDto> {
    ...
    @UseGuards(AuthGuard)
    @Patch(':id/password')
    async changePassword(@Param('id') id: string, @Body() body: ChangePasswordDto, @CurrentUser() currentUser: User): Promise<void> {
        if (currentUser.id.toString(10) !== id) {
            throw new ForbiddenException();
        }

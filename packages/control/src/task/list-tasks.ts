...
const NextTokenSchema = z.object({
  id: z.number(),
  priority: z.enum([
    TaskPriority.LOW.toString(),
    TaskPriority.MEDIUM.toString(),
...
export async function listTasks(
  ...
  if (options.pageSize && options.nextPageToken != null) {
    const jsonString = Buffer.from(options.nextPageToken, 'base64url').toString(
      'utf-8',
    );
    const parsedJson = JSON.parse(jsonString);
    const parsedToken = NextTokenSchema.parse(parsedJson);
    query = query.andWhere('(task.priority, task.id) < (:priority, :id)', {
  ...
  const page =
    options.pageSize != null ? tasks.slice(0, options.pageSize) : tasks;
